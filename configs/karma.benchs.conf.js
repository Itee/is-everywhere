...
function CreateKarmaBenchmarkConfiguration ( config ) {
    ...
    config.set( {
        ...
        benchmarkReporter: {
            ...
            formatSuiteSummary: ( benchmarks, benchConfig ) => {


                const _benchmarks = benchmarks.map( data => data.benchmark )
                _benchmarks.sort( ( a, b ) => {
                ...
                const fastest       = _benchmarks[ 0 ]
                const slowest       = _benchmarks[ _benchmarks.length - 1 ]
                const speedIncrease = ( ( fastest.hz - slowest.hz ) / slowest.hz ) * 100
                const summary       = `${ fastest.name } is ${ Math.round( speedIncrease ) }% fastest than ${ slowest.name }`
                const terminalWidth = benchConfig.terminalWidth
                const padLength     = Math.round( terminalWidth - ( ( terminalWidth - summary.length ) / 2 ) )

                return `${ EOL }${ summary.padStart( padLength, ' ' ) }${ EOL }${ EOL }`
            }
        },
